...
if (! function_exists('mcai_activation_handler')) {
    ...
    function mcai_activation_handler()
    {
        ...
        update_option('mcai_db_version', '2.0');
...
if (! function_exists('mcai_admin_notices')) {
    ...
    function mcai_admin_notices()
    {
        ...
        if (! get_option('mcai_dismiss_welcome_notice', false)) {
            $main_url = admin_url('admin.php?page=cloud-auto-importer');
?>
            <div class="notice notice-info is-dismissible" data-notice="mcai_welcome">
                <p>
                    ...
                    <?php esc_html_e('Click "Connect Google Drive" below to start importing images from Google Drive.', 'michael-cloud-image-auto-importer'); ?>
                </p>
                <p>
                    <a href="<?php echo esc_url($main_url); ?>" class="button button-primary">
                        <?php esc_html_e('Go to Plugin Page', 'michael-cloud-image-auto-importer'); ?>
                    </a>
                </p>
            </div>
        <?php
        ...
        // Check connection status - Show if not connected
        if (file_exists(MCAI_PLUGIN_PATH . 'includes/class-google-drive.php')) {
        ...
        ?>
                    <div class="notice notice-warning is-dismissible">
                        <p>
                            <strong><?php esc_html_e('Michael Cloud Image Auto Importer', 'michael-cloud-image-auto-importer'); ?></strong>
                            <?php
                            echo wp_kses_post(
                                sprintf(
                                    /* translators: %s: main page URL */
                                    __('Please <a href="%s">connect to Google Drive</a> to start importing images.', 'michael-cloud-image-auto-importer'),
                                    esc_url($main_url)
                                )
                            );
                            ?>
        ...
        if (file_exists($temp_dir) && is_dir($temp_dir)) {
            ...
            if ($files) {
                foreach ($files as $file) {
                    ...
                    if ($file_real !== false && $temp_real !== false && 0 === strpos($file_real, $temp_real)) {
            ...
            foreach ($subdirs as $subdir) {
                ...
                $temp_real = realpath($temp_dir);
                if ($subdir_real !== false && $temp_real !== false && 0 === strpos($subdir_real, $temp_real)) {
...
if (! function_exists('mcai_clear_all_cache')) {
    ...
    function mcai_clear_all_cache()
    {
        ...
        // Clear object cache group (WordPress 6.1+)
 ...
 * @since 2.0.0
 */
add_action(MCAI_HOOK_PREFIX . 'google_drive_connected', function($user_token) {
    // Developers can hook into this
}, 10, 1);