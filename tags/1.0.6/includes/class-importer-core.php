 ...
 * Handles file processing with NO usage limits
 *
...
class MCAI_Importer_Core
{
    ...
    public function start_import($folder_url, $import_id, $settings = array())
    {
        ...
        if (0 === $total_files) {
            return array(
                ...
                'import_id'    => $import_id,
                'total_files'  => 0,
                'message'      => __('No files found in folder', 'michael-cloud-image-auto-importer'),
            );
        }
    ...
    private function process_batch($import_id, $batch_size = 25)
    {
        ...
        if ($files_to_process <= 0) {
            return array(
                'success' => false,
                'message' => __('No files to process', 'michael-cloud-image-auto-importer'),
            );
        }
    ...
    private function process_single_file($file_data, $settings, $import_id)
    {
        ...
        if ($settings['skip_duplicates']) {
            ...
            if ($duplicate_check['is_duplicate']) {
                ...
                // Clean up temp file using WP_Filesystem.
    ...
    private function safe_delete_temp_file($temp_file)
    {
        ...
        if (WP_Filesystem() && $wp_filesystem->exists($temp_file)) {
            return $wp_filesystem->delete($temp_file);
        }
    ...
    private function compress_image($file_path, $settings)
    {
        ...
        if (! WP_Filesystem() || ! $wp_filesystem->exists($file_path)) {
            return array(
                'success' => false,
                'message' => __('File not found or filesystem error', 'michael-cloud-image-auto-importer'),
            );
        }

        // Get original file size.
        $original_size = $wp_filesystem->size($file_path);
        ...
        $compressed_size = $wp_filesystem->size($result['path']);

        /* translators: %s: compression result details (optional context) */
        if ($compressed_size >= $original_size) {
            // Compression didn't help, use original.
            $wp_filesystem->delete($result['path']);
            /* translators: %s: compression result details (optional context) */
            return array(
                /* translators: %s: compression result details (optional context) */
                'success' => false,
    ...
    {
        $allowed_extensions = apply_filters('mcai_allowed_extensions', 
        ...
        return isset($expected_mimes[$extension]) && 
               $expected_mimes[$extension] === $mime_type;
    }
